...
router.post("/edit", (req, res) => {
  ...
  form.parse(req, function (err, fields, files) {
    ...
    if (important == undefined) important = 0;
    else important = 1;
    let attachments = [];
...
router.post("/important_toggle", function (req, res) {
  ...
  form.parse(req, function (err, fields, files) {
    ...
    var important = 1;
    if (fields.important == "0") important = 0;
      ...
      .then((notices) => {
        notices.forEach((notice) => {
          ...
          notice.openDate =
            d1.getDate() + "/" + (d1.getMonth() + 1) + "/" + d1.getFullYear();
          if (
            notice.timestamp >= new Date(start_date) &&
            notice.timestamp <= new Date(end_date)
          ) {
