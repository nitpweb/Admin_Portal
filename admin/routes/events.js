        ...
        db.find({
            ...
            .then(events => {
                events.forEach(event => {
                    ...
                    event.closeDate = d.getDate() + '/' + (d.getMonth() + 1) + '/' + d.getFullYear();
                    var d1 = new Date(event.openDate)
                    event.openDate = d1.getDate() + '/' + (d1.getMonth() + 1) + '/' + d1.getFullYear();
                })
                events.sort(compare);
                // console.log(events);
                res.render('events', {
                ...
                .then(events => {
                    events.forEach(event => {
                        ...
                        if (event.id == file_id_here) {
                            var event_obj = new Event(file_id_here, title, attachments, req.session.user.id + "", new Date(open_date).getTime(), new Date(close_date).getTime(), venue, doclink)
        ...
        db.find({
            ...
            .then(events => {
                events.forEach(event => {
                    ...
                    var d1 = new Date(event.openDate)
                    event.openDate = d1.getDate() + '/' + (d1.getMonth() + 1) + '/' + d1.getFullYear();
                    if (event.timestamp >= new Date(start_date) && event.timestamp <= new Date(end_date)) {
